...
def match(caption: str, records: list, min_overlap: int = 4) -> int | None:
    """Best views value for a post caption via token overlap; None if too uncertain."""
    ct = tokens(caption)
    scored = [(len(ct & tokens(r["title"])), r["views"]) for r in records]
    ov, views = max(scored, default=(0, None))
    return views if ov >= min_overlap else None
